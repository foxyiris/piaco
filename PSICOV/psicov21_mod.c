...
/* Original code is a bit modified to accept chimeric alignments */
/* by Yoshinori Fukasawa */
/* Version 2.1d1 - Last Edit 9/Sep/16 */
...
void           *reallocvec(int *op, int columns, int size)
{
    void  *p;
    p = calloc(columns, size);

    if (p == NULL)
	fail("reallocvec: calloc failed!");
...
{
  int             a, b, i, j, k, seqlen, nids, s, nseqs, ncon, opt, ndim, filtflg=0, approxflg=0, initflg=0, apcflg=1, maxit=10000, npair, nnzero, niter, jerr, shrinkflg=1, rawscflg = 1, pseudoc = 1, minseqsep = 5, overrideflg=0, mapcflg=0, adjustflg=0;
...
#pragma omp critical 
      ...
      if (wtsum < MINEFSEQS && !overrideflg) {
          ...
          /* very ad-hoc hack for filtering */
          /*
          if(1){
            printf("Computable with default");
            exit(0);
          }
          */
...
//	    printf("New rfact = %f\n", rfact);
	...
	for (j=i+minseqsep; j<seqlen; j++)
		...
		if(i < mapcflg && j < mapcflg){
		  sclist[ncon].sc = pcmat[i][j] - pcsum[i] * pcsum[j]/ SQR(mapcflg - 1.0) / pcmean;
		} else if(i >= mapcflg && j >= mapcflg) {
		  sclist[ncon].sc = pcmat[i][j] - pcsum[i] * pcsum[j]/ SQR(seqlen - mapcflg - 1.0) / pcmean;
		} else {
